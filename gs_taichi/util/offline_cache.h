...
inline LoadMetadataError load_metadata_with_checking(
  ...
  if (!gs_taichi::path_exists(filepath)) {
 ...
 public:
  static void run(const CacheCleanerConfig &config) {
    ...
    const auto metadata_file =
        gs_taichi::join_path(path, config.metadata_filename);
    const auto debugging_metadata_file =
        gs_taichi::join_path(path, config.debugging_metadata_filename);

    if (policy == (std::size_t)NotClean) {
      return;
    }
    if (!gs_taichi::path_exists(path)) {
      return;
    }
    ...
    // 1. Remove/Update metadata files
    {
      std::string lock_path =
          gs_taichi::join_path(path, config.metadata_lock_name);
      ...
      if (error == Error::kFileNotFound) {
        ...
        if (policy &
          ...
          if (gs_taichi::remove(metadata_file)) {
            gs_taichi::remove(debugging_metadata_file);
            Utils::remove_other_files(config);
            bool success = gs_taichi::traverse_directory(
                config.path, [&config](const std::string &name, bool is_dir) {
                  if (!is_dir && Utils::is_valid_cache_file(config, name)) {
                    gs_taichi::remove(gs_taichi::join_path(config.path, name));
                  }
                });
            TI_ASSERT(success);
          }
      ...
      if (cmp) {
        ...
        if (cache_data.kernels.empty()) {  // Remove
          ok_rm_meta = gs_taichi::remove(metadata_file);
          gs_taichi::remove(debugging_metadata_file);
          Utils::remove_other_files(config);
        } else {  // Update
    ...
    if (ok_rm_meta) {
      ...
      for (const auto &f : files_to_rm) {
        auto file_path = gs_taichi::join_path(path, f);
        gs_taichi::remove(file_path);
      }
    }
  }
};
