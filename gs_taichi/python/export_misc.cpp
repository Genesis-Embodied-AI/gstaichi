...
#include "gs_taichi/math/math.h"
#include "gs_taichi/platform/cuda/detect_cuda.h"
#include "gs_taichi/program/py_print_buffer.h"
#include "gs_taichi/python/exception.h"
#include "gs_taichi/python/export.h"
#include "gs_taichi/python/memory_usage_monitor.h"
#include "gs_taichi/system/benchmark.h"
#include "gs_taichi/system/hacked_signal_handler.h"
...
#ifdef TI_WITH_DX12
#include "gs_taichi/rhi/dx12/dx12_api.h"
#endif
...
#define TI_EXPORT_LOGGING(X)               \
  m.def(#X, [](const std::string &msg) {   \
    gs_taichi::Logger::get_instance().X(msg); \
  });

  m.def("flush_log", []() { gs_taichi::Logger::get_instance().flush(); });
...
#ifdef TI_WITH_METAL
  m.def("with_metal", gs_taichi::lang::metal::is_metal_api_available);
#else
  m.def("with_metal", []() { return false; });
#endif
#ifdef TI_WITH_OPENGL
  m.def("with_opengl", gs_taichi::lang::opengl::is_opengl_api_available,
        py::arg("use_gles") = false);
#else
  m.def("with_opengl", [](bool use_gles) { return false; });
#endif
#ifdef TI_WITH_VULKAN
  m.def("with_vulkan", gs_taichi::lang::vulkan::is_vulkan_api_available);
...
#endif
#ifdef TI_WITH_DX11
  m.def("with_dx11", gs_taichi::lang::directx11::is_dx_api_available);
#else
  m.def("with_dx11", []() { return false; });
#endif
#ifdef TI_WITH_DX12
  m.def("with_dx12", gs_taichi::lang::directx12::is_dx12_api_available);
#else
