...
#include "gs_taichi/codegen/codegen_utils.h"
...
#include "gs_taichi/util/line_appender.h"
#include "gs_taichi/codegen/spirv/kernel_utils.h"
#include "gs_taichi/codegen/spirv/spirv_ir_builder.h"
#include "gs_taichi/ir/transforms.h"
#include "gs_taichi/math/arithmetic.h"
#include "gs_taichi/codegen/ir_dump.h"
...
class TaskCodegen : public IRVisitor {
 public:
  ...
  void visit(ReturnStmt *stmt) override {
    ...
    std::function<void(const gs_taichi::lang::Type *, int &, std::vector<int> &)>
        calc_indices_and_store = [&](const gs_taichi::lang::Type *type, int &index,
                                     std::vector<int> &indices) {
          if (auto struct_type = type->cast<gs_taichi::lang::StructType>()) {
            for (int i = 0; i < struct_type->elements().size(); ++i) {
              ...
              indices.pop_back();
                         ...
                         type->cast<gs_taichi::lang::TensorType>()) {
 ...
 private:
  ...
  void compile_args_struct() {
    ...
    std::unordered_map<std::vector<int>, const tinyir::Type *,
                       hashing::Hasher<std::vector<int>>>
        element_types;
    std::unordered_map<std::vector<int>, const gs_taichi::lang::Type *,
                       hashing::Hasher<std::vector<int>>>
