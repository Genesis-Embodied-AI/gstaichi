...
#include "gs_taichi/rhi/device_capability.h"
...
class fmt::formatter<gs_taichi::lang::RhiResult> {
 public:
  ...
  constexpr auto format(gs_taichi::lang::RhiResult const &res,
                        Context &ctx) const {
    return format_to(ctx.out(), gs_taichi::lang::rhi_result_to_string(res));
  }
};
