...
#include "gs_taichi/common/dynamic_loader.h"
#include "gs_taichi/rhi/cuda/cuda_context.h"
#include "gs_taichi/util/environ_config.h"
...
#include "gs_taichi/rhi/cuda/cuda_driver_functions.inc.h"
...
#define PER_CUSPARSE_FUNCTION(name, symbol_name, ...) \
  ...
  name.set_lock(&lock_);                              \
  name.set_names(#name, #symbol_name);
#include "gs_taichi/rhi/cuda/cusparse_functions.inc.h"
...
#define PER_CUSOLVER_FUNCTION(name, symbol_name, ...) \
  ...
  name.set_lock(&lock_);                              \
  name.set_names(#name, #symbol_name);
#include "gs_taichi/rhi/cuda/cusolver_functions.inc.h"
...
#define PER_CUBLAS_FUNCTION(name, symbol_name, ...) \
  ...
  name.set_lock(&lock_);                            \
  name.set_names(#name, #symbol_name);
#include "gs_taichi/rhi/cuda/cublas_functions.inc.h"
