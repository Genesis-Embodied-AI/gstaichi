...
#include "gs_taichi/ir/expression_printer.h"
#include "gs_taichi/ir/statements.h"
#include "gs_taichi/program/program.h"
#include "gs_taichi/common/exceptions.h"
...
void UnaryOpExpression::type_check(const CompileConfig *config) {
  ...
  */
  ...
  if (type == UnaryOpType::frexp) {
    ...
    elements.push_back(
        {gs_taichi::lang::TypeFactory::get_instance().get_primitive_int_type(
             32, /*is_signed=*/true),
         "exponent", (size_t)data_type_size(operand_primitive_type)});
    ret_type =
        gs_taichi::lang::TypeFactory::get_instance().get_struct_type(elements);
  ...
  if (operand_type->is<TensorType>()) {
    ret_type = gs_taichi::lang::TypeFactory::get_instance().get_tensor_type(
