...
Expr cast(const Expr &input) {
  return gs_taichi::lang::cast(input, get_data_type<T>());
}
...
Expr bit_cast(const Expr &input) {
  return gs_taichi::lang::bit_cast(input, get_data_type<T>());
}
...
template <typename T>
Expr expr_rand() {
  return gs_taichi::lang::expr_rand(get_data_type<T>());
}
