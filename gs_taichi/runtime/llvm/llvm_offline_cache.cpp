...
#include "gs_taichi/common/version.h"
...
static std::string get_llvm_cache_metadata_file_path(const std::string &dir) {
  return gs_taichi::join_path(dir, std::string(kMetadataFilename) + ".tcb");
}

static std::string get_llvm_cache_metadata_json_file_path(
    const std::string &dir) {
  return gs_taichi::join_path(dir, std::string(kMetadataFilename) + ".json");
}
...
struct CacheCleanerUtils<LlvmOfflineCache> {
  ...
  static bool save_metadata(const CacheCleanerConfig &config,
    ...
    write_to_binary_file(
        data, gs_taichi::join_path(config.path, config.metadata_filename));
    return true;
  }
  ...
  static bool save_debugging_metadata(const CacheCleanerConfig &config,
    ...
    ts.write_to_file(
        gs_taichi::join_path(config.path, config.debugging_metadata_filename));
    return true;
  }
...
bool LlvmOfflineCacheFileReader::load_meta_data(
  ...
  if (!gs_taichi::path_exists(tcb_path)) {
  ...
  std::string lock_path =
      gs_taichi::join_path(cache_file_path, kMetadataFileLockName);
...
bool LlvmOfflineCacheFileReader::get_kernel_cache(
  ...
  if (!data.module) {
    std::string filename_prefix = gs_taichi::join_path(path_, key);
  ...
  if (!verified) {
    for (const auto &f : get_possible_llvm_cache_filename_by_key(key)) {
      gs_taichi::remove(gs_taichi::join_path(path_, f));
    }
  }
...
void LlvmOfflineCacheFileWriter::dump(const std::string &path,
  ...
  gs_taichi::create_directories(path);
  ...
  for (auto iter = data_.kernels.begin(); iter != data_.kernels.end(); ++iter) {
    ...
    std::string filename_prefix = gs_taichi::join_path(path, k);
    {
...

  {
    ...
    std::string lock_path = gs_taichi::join_path(path, kMetadataFileLockName);
    if (!lock_with_file(lock_path)) {
      TI_WARN(
