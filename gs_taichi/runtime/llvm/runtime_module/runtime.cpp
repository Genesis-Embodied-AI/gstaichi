...
#include "gs_taichi/inc/cuda_kernel_utils.inc.h"
...
#include "gs_taichi/runtime/llvm/runtime_module/atomic.h"
...
struct ListManager {
  ...
  ListManager(LLVMRuntime *runtime,
    ...
    lock = 0;
    num_elements = 0;
    log2chunk_num_elements = gs_taichi::log2int(num_elements_per_chunk);
  }
...
void runtime_get_memory_requirements(Ptr result_buffer,
  ...
  if (use_preallocated_buffer) {
    size += gs_taichi::iroundup(i64(sizeof(LLVMRuntime)), taichi_page_size);
  }

  size +=
      gs_taichi::iroundup(i64(taichi_global_tmp_buffer_size), taichi_page_size);
  size += gs_taichi::iroundup(i64(sizeof(RandState)) * num_rand_states,
                           taichi_page_size);
...
void runtime_initialize(
  ...
  if (preallocated_size) {
    ...
    preallocated_buffer +=
        gs_taichi::iroundup(sizeof(LLVMRuntime), taichi_page_size);
  } else {
    runtime =
