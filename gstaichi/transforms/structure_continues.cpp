...
class StructureContinues {
 public:
  static bool run(IRNode *root) {
    ...
    // because BasicStmtVisitor descends into them without calling the test function
    ...
    TI_INFO("[structure_continues] Found {} offloaded loops to check", offloads.size());
 ...
 private:
  static bool restructure_offload(OffloadedStmt *offload) {
    ...
    auto continues = irpass::analysis::gather_statements(offload->body.get(), [offload](Stmt *s) {
      ...
      TI_INFO("[structure_continues]   Found continue: scope={}, scope==offload={}", 
              (void*)cont->scope, cont->scope == offload);
      
      // Check if this continue is inside a nested loop but targets the offload
      // This happens after inlining when a function return becomes a continue
      if (cont->scope == offload) {
        // Find if there's an inner loop between the continue and the offload
        auto *inner_loop = find_innermost_loop(cont);
        TI_INFO("[structure_continues]     inner_loop={}, inner_loop==offload={}", 
                (void*)inner_loop, inner_loop == offload);
        if (inner_loop && inner_loop != offload) {
          TI_INFO("[structure_continues]     -> This continue needs restructuring!");
          return true;
        }
      }
    ...
    TI_INFO("[structure_continues] Found {} continues to restructure in offload", continues.size());
    ...
    for (auto &[inner_loop, loop_continues] : loop_to_continues) {
      ...
      for (auto *cont_stmt : loop_continues) {
        transform_continue(cont_stmt->as<ContinueStmt>(), flag_var, inner_loop, offload);
      }
    ...
    for (auto it = loops_and_flags.rbegin(); it != loops_and_flags.rend(); ++it) {
  ...
  static void transform_continue(ContinueStmt *cont, Stmt *flag_var, Stmt *inner_loop, OffloadedStmt *offload) {
  ...
  static void add_flag_check_after_loop(Stmt *loop, Stmt *flag_var, OffloadedStmt *offload) {
    ...
    for (size_t i = 0; i < stmts.size(); i++) {
      if (contains_stmt(stmts[i].get(), loop)) {
        ...
        TI_INFO("Found loop at index {} in offload body (total {} stmts)", i, stmts.size());
        
        // We need to:
        // 1. Insert flag reset BEFORE the loop (at position i)
        // 2. Insert flag check AFTER the loop (at position i+3, since we added 2 stmts before)
        ...
        auto reset_before = Stmt::make<LocalStoreStmt>(flag_var, false_ptr_before);
        ...
        // Now insert them using Block::insert() which properly sets parent pointers
        // Insert in reverse order of position to avoid index shifting issues
        // After loop: insert flag check (will be at position i+1 after we insert before)
