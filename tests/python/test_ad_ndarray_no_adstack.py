...
@test_utils.test(arch=archs_support_ndarray_ad)
@pytest.mark.parametrize("default_fp", [ti.f32, ti.f64])
def test_ad_sum_static(default_fp):
    assert ti.lang is not None
    arch = ti.lang.impl.current_cfg().arch
    if sys.platform == "darwin" and arch in [ti.metal, ti.vulkan] and default_fp == ti.f64:
        pytest.skip("fp64 not supported on mac gpu")
...
@test_utils.test(arch=archs_support_ndarray_ad)
@pytest.mark.parametrize("default_fp", [ti.f32, ti.f64])
def test_ad_sum_local_atomic_static(default_fp) -> None:
    assert ti.lang is not None
    arch = ti.lang.impl.current_cfg().arch
    if sys.platform == "darwin" and arch in [ti.metal, ti.vulkan] and default_fp == ti.f64:
        pytest.skip("fp64 not supported on mac gpu")
...
@pytest.mark.parametrize("default_fp", [ti.f32, ti.f64])
@test_utils.test(arch=archs_support_ndarray_ad)
def test_ad_power_static(default_fp):
    assert ti.lang is not None
    arch = ti.lang.impl.current_cfg().arch
    if sys.platform == "darwin" and arch in [ti.metal, ti.vulkan] and default_fp == ti.f64:
        pytest.skip("fp64 not supported on mac gpu")
...
@pytest.mark.parametrize("default_fp", [ti.f32, ti.f64])
@test_utils.test(arch=archs_support_ndarray_ad)
def test_ad_fibonacci_static(default_fp):
    assert ti.lang is not None
    arch = ti.lang.impl.current_cfg().arch
    if sys.platform == "darwin" and arch in [ti.metal, ti.vulkan] and default_fp == ti.f64:
        pytest.skip("fp64 not supported on mac gpu")
...
@pytest.mark.parametrize("default_fp", [ti.f32, ti.f64])
@test_utils.test(arch=archs_support_ndarray_ad)
def test_ad_fibonacci_index_static(default_fp):
    assert ti.lang is not None
    arch = ti.lang.impl.current_cfg().arch
    if sys.platform == "darwin" and arch in [ti.metal, ti.vulkan] and default_fp == ti.f64:
        pytest.skip("fp64 not supported on mac gpu")
...
@pytest.mark.parametrize("default_fp", [ti.f32, ti.f64])
@test_utils.test(arch=archs_support_ndarray_ad)
def test_integer_stack_static(default_fp):
    assert ti.lang is not None
    arch = ti.lang.impl.current_cfg().arch
    if sys.platform == "darwin" and arch in [ti.metal, ti.vulkan] and default_fp == ti.f64:
        pytest.skip("fp64 not supported on mac gpu")
...
@test_utils.test(arch=archs_support_ndarray_ad)
@pytest.mark.parametrize("default_fp", [ti.f32, ti.f64])
def test_double_for_loops_static(default_fp):
    assert ti.lang is not None
    arch = ti.lang.impl.current_cfg().arch
    if sys.platform == "darwin" and arch in [ti.metal, ti.vulkan] and default_fp == ti.f64:
        pytest.skip("fp64 not supported on mac gpu")
...
@test_utils.test(arch=archs_support_ndarray_ad)
@pytest.mark.parametrize("default_fp", [ti.f32, ti.f64])
def test_double_for_loops_more_nests_static(default_fp):
    assert ti.lang is not None
    arch = ti.lang.impl.current_cfg().arch
    if sys.platform == "darwin" and arch in [ti.metal, ti.vulkan] and default_fp == ti.f64:
        pytest.skip("fp64 not supported on mac gpu")
    ...
    def double_for(a: ti.types.ndarray(), b: ti.types.ndarray(), c: ti.types.ndarray(), f: ti.types.ndarray()):
        for i in range(N):
            for k in ti.static(range(N // 2)):
                weight = 1.0
                for j in ti.static(range(N * 2)):
                    if j < c[i, k]:
                        weight *= a[i]
                s = 0.0
                for j in ti.static(range(N * 3)):
                    if j < c[i, k] * 2:
                        s += weight + b[i]
                f[i, k] = s
...
@test_utils.test(arch=archs_support_ndarray_ad)
@pytest.mark.parametrize("default_fp", [ti.f32, ti.f64])
def test_complex_body_static(default_fp):
    assert ti.lang is not None
    arch = ti.lang.impl.current_cfg().arch
    if sys.platform == "darwin" and arch in [ti.metal, ti.vulkan] and default_fp == ti.f64:
        pytest.skip("fp64 not supported on mac gpu")
...
@pytest.mark.parametrize("default_fp", [ti.f32, ti.f64])
@test_utils.test(arch=archs_support_ndarray_ad)
def test_mixed_inner_loops_static(default_fp) -> None:
    assert ti.lang is not None
    arch = ti.lang.impl.current_cfg().arch
    if sys.platform == "darwin" and arch in [ti.metal, ti.vulkan] and default_fp == ti.f64:
        pytest.skip("fp64 not supported on mac gpu")
...
@test_utils.test(arch=archs_support_ndarray_ad)
@pytest.mark.parametrize("default_fp", [ti.f32, ti.f64])
def test_mixed_inner_loops_tape_static(default_fp) -> None:
    assert ti.lang is not None
    arch = ti.lang.impl.current_cfg().arch
    if sys.platform == "darwin" and arch in [ti.metal, ti.vulkan] and default_fp == ti.f64:
        pytest.skip("fp64 not supported on mac gpu")
