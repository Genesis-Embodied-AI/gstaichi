...
TEST(IsSameTypeTest, IsSameType) {
  ...
  for (auto t1 : types) {
    for (auto t2 : types) {
      ASSERT_TRUE(gs_taichi::lang::is_same_type(t1, t2));
      ASSERT_TRUE(gs_taichi::lang::is_same_type(llvm::PointerType::get(t1, 0),
  ...
  auto func2 = llvm::FunctionType::get(
      ...
      false);
  ASSERT_TRUE(gs_taichi::lang::is_same_type(func1, func2));
}
