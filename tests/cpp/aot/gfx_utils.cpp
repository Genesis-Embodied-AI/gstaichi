...
static void write_devalloc(gs_taichi::lang::DeviceAllocation &alloc,
                           const void *data,
                           size_t size) {
...
static void load_devalloc(gs_taichi::lang::DeviceAllocation &alloc,
                          void *data,
                          size_t size) {
  void *device_arr_ptr{nullptr};
  TI_ASSERT(alloc.device->map(alloc, &device_arr_ptr) ==
...
void view_devalloc_as_ndarray(Device *device_) {
  const int size = 40;
  gs_taichi::lang::Device::AllocParams alloc_params;
  alloc_params.host_write = true;
  alloc_params.size = size * sizeof(int);
  alloc_params.usage = gs_taichi::lang::AllocUsage::Storage;
...
void run_dense_field_kernel(Arch arch, gs_taichi::lang::Device *device) {
  // API based on proposal https://github.com/taichi-dev/taichi/issues/3642
  // Initialize program
  gs_taichi::uint64 *result_buffer{nullptr};
  result_buffer = (gs_taichi::uint64 *)HostMemoryPool::get_instance().allocate(
      sizeof(gs_taichi::uint64) * taichi_result_buffer_entries, 8);

  // Create runtime
  gfx::GfxRuntime::Params params;
  params.device = device;
  auto gfx_runtime =
      std::make_unique<gs_taichi::lang::gfx::GfxRuntime>(std::move(params));
...
void run_kernel_test1(Arch arch, gs_taichi::lang::Device *device) {
  // API based on proposal https://github.com/taichi-dev/taichi/issues/3642
  // Initialize program
  gs_taichi::uint64 *result_buffer{nullptr};
  result_buffer = (gs_taichi::uint64 *)HostMemoryPool::get_instance().allocate(
...
void run_kernel_test2(Arch arch, gs_taichi::lang::Device *device) {
  gs_taichi::uint64 *result_buffer{nullptr};
  result_buffer = (gs_taichi::uint64 *)HostMemoryPool::get_instance().allocate(
      sizeof(gs_taichi::uint64) * taichi_result_buffer_entries, 8);
...
void run_cgraph1(Arch arch, gs_taichi::lang::Device *device_) {
  // API based on proposal https://github.com/taichi-dev/taichi/issues/3642
  // Initialize  program
  gs_taichi::uint64 *result_buffer{nullptr};
  result_buffer = (gs_taichi::uint64 *)HostMemoryPool::get_instance().allocate(
...
void run_cgraph2(Arch arch, gs_taichi::lang::Device *device_) {
  // API based on proposal https://github.com/taichi-dev/taichi/issues/3642
  // Initialize program
  gs_taichi::uint64 *result_buffer{nullptr};
  result_buffer = (gs_taichi::uint64 *)HostMemoryPool::get_instance().allocate(
...
void run_mpm88_graph(Arch arch, gs_taichi::lang::Device *device_) {
  ...
  gs_taichi::lang::DeviceAllocation devalloc_x;
  EXPECT_EQ(device_->allocate_memory(alloc_params, &devalloc_x),
            RhiResult::success);
  auto x = gs_taichi::lang::Ndarray(devalloc_x, gs_taichi::lang::PrimitiveType::f32,
                                 {NR_PARTICLES}, {2});

  gs_taichi::lang::DeviceAllocation devalloc_v;
  EXPECT_EQ(device_->allocate_memory(alloc_params, &devalloc_v),
            RhiResult::success);
  auto v = gs_taichi::lang::Ndarray(devalloc_v, gs_taichi::lang::PrimitiveType::f32,
                                 {NR_PARTICLES}, {2});

  alloc_params.size = NR_PARTICLES * 3 * sizeof(float);
  gs_taichi::lang::DeviceAllocation devalloc_pos;
  EXPECT_EQ(device_->allocate_memory(alloc_params, &devalloc_pos),
            RhiResult::success);
  auto pos = gs_taichi::lang::Ndarray(
      devalloc_pos, gs_taichi::lang::PrimitiveType::f32, {NR_PARTICLES}, {3});

  alloc_params.size = NR_PARTICLES * sizeof(float) * 2 * 2;
  gs_taichi::lang::DeviceAllocation devalloc_C;
  EXPECT_EQ(device_->allocate_memory(alloc_params, &devalloc_C),
            RhiResult::success);
  auto C = gs_taichi::lang::Ndarray(devalloc_C, gs_taichi::lang::PrimitiveType::f32,
                                 {NR_PARTICLES}, {2, 2});

  alloc_params.size = NR_PARTICLES * sizeof(float);
  gs_taichi::lang::DeviceAllocation devalloc_J;
  EXPECT_EQ(device_->allocate_memory(alloc_params, &devalloc_J),
            RhiResult::success);
  auto J = gs_taichi::lang::Ndarray(devalloc_J, gs_taichi::lang::PrimitiveType::f32,
                                 {NR_PARTICLES});

  alloc_params.size = N_GRID * N_GRID * 2 * sizeof(float);
  gs_taichi::lang::DeviceAllocation devalloc_grid_v;
  EXPECT_EQ(device_->allocate_memory(alloc_params, &devalloc_grid_v),
            RhiResult::success);
  auto grid_v = gs_taichi::lang::Ndarray(
      devalloc_grid_v, gs_taichi::lang::PrimitiveType::f32, {N_GRID, N_GRID}, {2});

  alloc_params.size = N_GRID * N_GRID * sizeof(float);
  gs_taichi::lang::DeviceAllocation devalloc_grid_m;
  EXPECT_EQ(device_->allocate_memory(alloc_params, &devalloc_grid_m),
            RhiResult::success);
  auto grid_m = gs_taichi::lang::Ndarray(
      devalloc_grid_m, gs_taichi::lang::PrimitiveType::f32, {N_GRID, N_GRID});

  std::unordered_map<std::string, gs_taichi::lang::aot::IValue> args;
  args.insert({"x", gs_taichi::lang::aot::IValue::create(x)});
  args.insert({"v", gs_taichi::lang::aot::IValue::create(v)});
  args.insert({"J", gs_taichi::lang::aot::IValue::create(J)});

  g_init->run(args);
  gfx_runtime->synchronize();

  args.insert({"C", gs_taichi::lang::aot::IValue::create(C)});
  args.insert({"grid_v", gs_taichi::lang::aot::IValue::create(grid_v)});
  args.insert({"grid_m", gs_taichi::lang::aot::IValue::create(grid_m)});
  args.insert({"pos", gs_taichi::lang::aot::IValue::create(pos)});
