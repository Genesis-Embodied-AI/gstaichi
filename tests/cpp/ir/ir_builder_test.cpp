...
TEST(IRBuilder, ExternalPtr) {
  ...
  auto tuple = prog->compile_kernel(prog->compile_config(),
...
TEST(IRBuilder, Ndarray) {
  ...
  auto tuple2 = prog->compile_kernel(prog->compile_config(),
                                     prog->get_device_caps(), *ker2);
  auto &compiled_kernel_data2 = tuple2.compiled_kernel_data;
