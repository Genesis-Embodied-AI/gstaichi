...
GfxRuntime::GfxRuntime(gs_taichi::Arch arch) : Runtime(arch) {
}

Error GfxRuntime::create_aot_module(const gs_taichi::io::VirtualDir *dir,
                                    TiAotModule &out) {
  gs_taichi::lang::gfx::AotModuleParams params{};
  params.dir = dir;
  params.runtime = &get_gfx_runtime();
  std::unique_ptr<gs_taichi::lang::aot::Module> aot_module =
      gs_taichi::lang::aot::Module::load(arch, params);
  if (aot_module->is_corrupted()) {
    return Error(TI_ERROR_CORRUPTED_DATA, "aot_module");
  }

  const gs_taichi::lang::DeviceCapabilityConfig &current_devcaps =
      params.runtime->get_ti_device()->get_caps();
  const gs_taichi::lang::DeviceCapabilityConfig &required_devcaps =
  ...
  for (const auto &pair : required_devcaps.devcaps) {
    ...
    if (current_version != required_version) {
      return Error(TI_ERROR_INCOMPATIBLE_MODULE,
                   gs_taichi::lang::to_string(pair.first).c_str());
    }
  }
...
void GfxRuntime::buffer_copy(const gs_taichi::lang::DevicePtr &dst,
                             const gs_taichi::lang::DevicePtr &src,
                             size_t size) {
  get_gfx_runtime().buffer_copy(dst, src, size);
}
void GfxRuntime::copy_image(const gs_taichi::lang::DeviceAllocation &dst,
...
void GfxRuntime::track_image(const gs_taichi::lang::DeviceAllocation &image,
                             gs_taichi::lang::ImageLayout layout) {
  get_gfx_runtime().track_image(image, layout);
}
void GfxRuntime::untrack_image(const gs_taichi::lang::DeviceAllocation &image) {
  get_gfx_runtime().untrack_image(image);
}
void GfxRuntime::transition_image(const gs_taichi::lang::DeviceAllocation &image,
                                  gs_taichi::lang::ImageLayout layout) {
