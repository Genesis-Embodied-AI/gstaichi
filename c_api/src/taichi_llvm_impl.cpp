...
#include "gs_taichi/rhi/cpu/cpu_device.h"

#ifdef TI_WITH_CUDA
#include "gs_taichi/rhi/cuda/cuda_device.h"
#include "gs_taichi/rhi/cuda/cuda_context.h"
#include "gs_taichi/runtime/cuda/kernel_launcher.h"
#endif

namespace capi {

LlvmRuntime::LlvmRuntime(gs_taichi::Arch arch) : Runtime(arch) {
  cfg_ = std::make_unique<gs_taichi::lang::CompileConfig>();
  cfg_->arch = arch;

  executor_ =
      std::make_unique<gs_taichi::lang::LlvmRuntimeExecutor>(*cfg_.get(), nullptr);
...
gs_taichi::lang::Device &LlvmRuntime::get() {
  gs_taichi::lang::Device *device = executor_->get_compute_device();
  return *device;
}

TiMemory LlvmRuntime::allocate_memory(
    const gs_taichi::lang::Device::AllocParams &params) {
  gs_taichi::lang::LLVMRuntime *llvm_runtime = executor_->get_llvm_runtime();
  gs_taichi::lang::LlvmDevice *llvm_device = executor_->llvm_device();
...
void LlvmRuntime::free_memory(TiMemory devmem) {
  const gs_taichi::lang::CompileConfig &config = executor_->get_config();
  // For memory allocated through Device::allocate_memory_runtime(),
  // the corresponding Device::free_memory() interface has not been
  // implemented yet...
  if (gs_taichi::arch_is_cpu(config.arch)) {
    TI_CAPI_NOT_SUPPORTED_IF(gs_taichi::arch_is_cpu(config.arch));
  }
...
TiAotModule LlvmRuntime::load_aot_module(const char *module_path) {
  ...
  std::unique_ptr<gs_taichi::lang::aot::Module> aot_module{nullptr};

  if (gs_taichi::arch_is_cpu(config.arch)) {
    gs_taichi::lang::cpu::KernelLauncher::Config cfg;
    cfg.executor = executor_.get();
    gs_taichi::lang::LLVM::AotModuleParams aot_params;
    aot_params.executor_ = executor_.get();
    aot_params.kernel_launcher =
        std::make_unique<gs_taichi::lang::cpu::KernelLauncher>(std::move(cfg));
    aot_params.module_path = module_path;
    aot_module = gs_taichi::lang::LLVM::make_aot_module(std::move(aot_params));
  } else {
#ifdef TI_WITH_CUDA
    TI_ASSERT(config.arch == gs_taichi::Arch::cuda);
    gs_taichi::lang::cuda::KernelLauncher::Config cfg;
    cfg.executor = executor_.get();
    gs_taichi::lang::LLVM::AotModuleParams aot_params;
    aot_params.executor_ = executor_.get();
    aot_params.kernel_launcher =
        std::make_unique<gs_taichi::lang::cuda::KernelLauncher>(std::move(cfg));
    aot_params.module_path = module_path;
    aot_module = gs_taichi::lang::LLVM::make_aot_module(std::move(aot_params));
#else
    TI_NOT_IMPLEMENTED;
  ...
  */
  auto *llvm_aot_module =
      dynamic_cast<gs_taichi::lang::LLVM::LlvmAotModule *>(aot_module.get());
  ...
  for (size_t i = 0; i < llvm_aot_module->get_num_snode_trees(); i++) {
    ...
    gs_taichi::lang::LLVM::allocate_aot_snode_tree_type(
...
void LlvmRuntime::buffer_copy(const gs_taichi::lang::DevicePtr &dst,
...
void ti_export_cpu_memory(TiRuntime runtime,
  ...
  if (((Runtime *)runtime)->arch != gs_taichi::Arch::x64 &&
      ((Runtime *)runtime)->arch != gs_taichi::Arch::arm64) {
  ...
  auto &cpu_device = static_cast<gs_taichi::lang::cpu::CpuDevice &>(device);
...
TI_DLL_EXPORT TiMemory TI_API_CALL ti_import_cpu_memory(TiRuntime runtime,
  ...
  gs_taichi::lang::DeviceAllocation device_alloc =
...
void ti_export_cuda_memory(TiRuntime runtime,
  ...
  TI_CAPI_ARGUMENT_NULL(memory);
  TI_CAPI_ARGUMENT_NULL(interop_info);
  if (((Runtime *)runtime)->arch != gs_taichi::Arch::cuda) {
  ...
  auto &device = llvm_runtime->get();
  auto &cuda_device = static_cast<gs_taichi::lang::cuda::CudaDevice &>(device);
...
#ifdef TI_WITH_CUDA
  gs_taichi::lang::CUDAContext::get_instance().set_stream(stream);
...
#ifdef TI_WITH_CUDA
  *stream = gs_taichi::lang::CUDAContext::get_instance().get_stream();
#else
  TI_NOT_IMPLEMENTED;
